...
contract Exchange is owned {
    ...
    /////////////////////////////
    // function getBuyOrderBook accepts one string variable "symbolName" and returns two arrays.
    function getBuyOrderBook(string symbolName) constant returns (uint[], uint[]) {
        uint8 tokenNameIndex = getSymbolIndexOrThrow(symbolName);
        uint[] memory arrPricesBuy = new uint[](tokens[tokenNameIndex].amountBuyPrices); // array for holding Buy Prices.
        uint[] memory arrVolumesBuy = new uint[](tokens[tokenNameIndex].amountBuyPrices); // array for holdng Buy Volumes.

        uint whilePrice = tokens[tokenNameIndex].lowestBuyPrice; // get the lowestBuyPrice.
        uint counter = 0;
        // check if there is at least one Buy Price offers.
        if (tokens[tokenNameIndex].curBuyPrice > 0) {
            while (whilePrice <= tokens[tokenNameIndex].curBuyPrice) {
                arrPricesBuy[counter] = whilePrice;
                uint volumeAtPrice = 0; // initialize our volumeAtPrice to zero,
                uint offers_key = 0; // initialize our offers key to zero

                offers_key = tokens[tokenNameIndex].buyBook[whilePrice].offers_key; // retrieve offers key from the buyBook.
                // loop through all the current & existing offers and increment our volumeAtPrice.
        ...
        return (arrPricesBuy, arrVolumesBuy); // return the Buy Prices array as well as the Buy Volumes array.
    }
    ...
    /////////////////////////////
    // function getSellOrderBook accepts one string variable "symbolName" and returns two arrays.
    function getSellOrderBook(string symbolName) constant returns (uint[], uint[]) {
      uint8 tokenNameIndex = getSymbolIndexOrThrow(symbolName);
      uint[] memory arrPricesSell = new uint[](tokens[tokenNameIndex].amountSellPrices); // array for holding Buy Prices.
      uint[] memory arrVolumesSell = new uint[](tokens[tokenNameIndex].amountSellPrices); // array for holdng Buy Volumes.
      uint sellWhilePrice = tokens[tokenNameIndex].highestSellPrice; // get the highestSellPrice.
      uint sellCounter = 0;
      // check if there is at least one Sell Price offers.
      if (tokens[tokenNameIndex].curSellPrice > 0) {
        // loop while the whilePrice is less than equal to the curSellPrice
        while (sellWhilePrice <= tokens[tokenNameIndex].curSellPrice) {
            // push the whilePrice into the arrPricesSell array.
            arrPricesSell[sellCounter] = sellWhilePrice;
            uint sellVolumeAtPrice = 0; // initialize our volumeAtPrice to zero,
            uint sell_offers_key = 0; // initialize our offers key to zero

            sell_offers_key = tokens[tokenNameIndex].sellBook[sellWhilePrice].offers_key; // retrieve offers key from the sellBook.
            // loop through all the current & existing sell offers and increment our volumeAtPrice.
            while (sell_offers_key <= tokens[tokenNameIndex].sellBook[sellWhilePrice].offers_length) {
                ...
                sell_offers_key++; // increment the offers_key.
            }
            // add the Sell price volumes into the arrVolumesSell array
            arrVolumesSell[sellCounter] = sellVolumeAtPrice;
            //next sellWhilePrice
            if (sellWhilePrice == tokens[tokenNameIndex].sellBook[sellWhilePrice].higherPrice) {
                break;
            }
            else {
        ...
        return (arrPricesSell, arrVolumesSell); // return the Buy Prices array as well as the Buy Volumes array.
    }
    ...
    function buyToken(string symbolName, uint priceInWei, uint amount) {
      ...
      if (tokens[tokenNameIndex].amountSellPrices == 0 || tokens[tokenNameIndex].curSellPrice > priceInWei) {
          ...
          // set the whilePrice to the curSellPrice.
          uint whilePrice = tokens[tokenNameIndex].curSellPrice;
          // set our volume(amountNecessary) to the incoming amount.
          uint amountNecessary = amount;
          // initialize the offers_key.
          uint offers_key;
          // loop while the whilePrice <= the priceInWei and the volume(amountNecessary) > 0.
          while (whilePrice <= priceInWei && amountNecessary > 0) {//we start with the smallest sell price.
              offers_key = tokens[tokenNameIndex].sellBook[whilePrice].offers_key; // set the offers_key fromthe sellBook.
              while (offers_key <= tokens[tokenNameIndex].sellBook[whilePrice].offers_length && amountNecessary > 0) {//and the first order (FIFO)
                  // save the volume for the required price; volumeAtPriceFromAddress.
                  ...
                  if (volumeAtPriceFromAddress <= amountNecessary) {
                      // get the total ether amount available; by multiplying the volume byt the Price.
                      total_amount_ether_available = volumeAtPriceFromAddress * whilePrice;

                      // the sender should have >= the total_amount_ether_available in his account.
                      ...
                      //first deduct the amount of ether(total_amount_ether_available) from our balance(balanceEthForAddress)
                      ...
                      // deduct the volume(volumeAtPriceFromAddress) from the total ether necessary(amountNecessary).
                      amountNecessary -= volumeAtPriceFromAddress;
                  }
                  else {
                      // check that our current volume is > the volume that is needed(amountNecessary).
                      require(tokens[tokenNameIndex].sellBook[whilePrice].offers[offers_key].amount > amountNecessary);//sanity

                      //our total amount of ether necessary is now the amountNecessary multiplied by the whilePrice
                      total_amount_ether_necessary = amountNecessary * whilePrice;
                      require(balanceEthForAddress[msg.sender] - total_amount_ether_necessary <= balanceEthForAddress[msg.sender]);

                      //first deduct the amount of ether(total_amount_ether_necessary) from our balance(balanceEthForAddress).
                      balanceEthForAddress[msg.sender] -= total_amount_ether_necessary;
                      ...
                      // deduct the amountNecessary from our total volume(amount).
                      ...
                      // add the total volume necessary to the tokenBalanceForAddress.
                      tokenBalanceForAddress[msg.sender][tokenNameIndex] += amountNecessary;

                      amountNecessary = 0; // set the volume(amountNecessary) to zero
    ...
    tokens[tokenIndex].buyBook[priceInWei].offers_length++; // increment our offers in the buy queue by one; this is initialized to zero by default.
    tokens[tokenIndex].buyBook[priceInWei].offers[tokens[tokenIndex].buyBook[priceInWei].offers_length] = Offer(amount, who);

    // check if this is our first order in the queue.
    // adjust the higher and lower order prices linked lists.
    if (tokens[tokenIndex].buyBook[priceInWei].offers_length == 1) {
        ...
        if (lowestBuyPrice == 0 || lowestBuyPrice > priceInWei) {
            if (curBuyPrice == 0) {
                ...
                tokens[tokenIndex].buyBook[priceInWei].higherPrice = priceInWei; // the higher price becomes the priceInWei & the
                tokens[tokenIndex].buyBook[priceInWei].lowerPrice = 0; // lower price becomes zero since this is the first item in the queue.
            } else {
                //or the lowest one
                tokens[tokenIndex].buyBook[lowestBuyPrice].lowerPrice = priceInWei; // set the lower price to our price that we are offering.
                tokens[tokenIndex].buyBook[priceInWei].higherPrice = lowestBuyPrice; // set the highest price to our new entry in the queue.
                tokens[tokenIndex].buyBook[priceInWei].lowerPrice = 0;
            }
            tokens[tokenIndex].lowestBuyPrice = priceInWei; // set out lowest buy price to our incoming priceInWei.
        }
        else if (curBuyPrice < priceInWei) {
            //the offer to buy is the highest one, we don't need to find the right spot (this is the LAST entry in the queue.)
            tokens[tokenIndex].buyBook[curBuyPrice].higherPrice = priceInWei; // take the current higher price and make it our current priceInWei.
            tokens[tokenIndex].buyBook[priceInWei].higherPrice = priceInWei; // set the higher price in the queue to the priceInWei.
            tokens[tokenIndex].buyBook[priceInWei].lowerPrice = curBuyPrice; // the lower price is our previous curBuyPrice.
            tokens[tokenIndex].curBuyPrice = priceInWei; // set our current buy price to our priceInWei, the price we're adding.
    ...
    function cancelOrder(string symbolName, bool isSellOrder, uint priceInWei, uint offerKey) public {
      uint8 symbolNameIndex = getSymbolIndexOrThrow(symbolName);
             // check if it is a sell order. work with the sell book.
             if (isSellOrder) {
                 // ensure that the person (offers[offerKey].who) that created the sell order is the same as the one cancelling the order.
                 ...
                 // check that the tokenBalanceForAddress + the tokensAmount is >= the tokenBalanceForAddress
             ...
             // else it is a buy order. work with the buy book.
             else {
                 // ensure that the person (offers[offerKey].who) that created the buy order is the same as the one cancelling the order.
                 // throw an exception if not the same.
